...
# 구조도 제목 — '3. 토지담보대출' 처럼 번호로 시작한다
_TITLE_HEAD = re.compile(r"^\d+\s*[.．]")
...
def insert_diagram(dst_slide, pptx_path: str):
    ...
    for sh in src.shapes:                      # 맨 바깥 도형만(그룹은 통째로)
        ...
        # ★'위쪽에 있으면 뺀다'로 거르면 안 된다. 위에서 시작해 아래로 꺾여 내려오는
        #   **긴 화살표가 통째로 사라진다**(3번 구조도의 '순위별 우선수익권 제공',
        #   1번은 아예 슬라이드 밖 T -0.44 에서 시작). 로고와 제목만 콕 집어 뺀다.
        if sh.shape_type == 13 and t < Emu(int(0.6 * EMU_IN)) \
                and l < Emu(int(2.5 * EMU_IN)) and w < Emu(int(3.0 * EMU_IN)):
            continue                            # 회사 로고
        if sh.has_text_frame and t < Emu(int(0.9 * EMU_IN)) \
                and l < Emu(int(1.5 * EMU_IN)) \
                and _TITLE_HEAD.match((sh.text_frame.text or "").strip()):
            continue                            # '3. 토지담보대출' 같은 구조도 제목
